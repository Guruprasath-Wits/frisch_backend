const coupon = require('../services/coupon.service');
...
exports.validate = (req, res) => {
    ...
    coupon.validate(couponCode, userId, (err, data) => {
        if (err) {
            return res.status(500).send({
                status: false,
                message: err.message || "Some error occurred while validating the coupon."
            });
        }
        res.send(data);
    });
...
exports.getAvailable = (req, res) => {
    ...
    coupon.getAvailable(userId, (err, data) => {
        if (err) {
            res.status(500).send({
                status: false,
                message: err.message || "Some error occurred while retrieving available coupons."
            });
        } else {
            // Format dates
            const formattedData = data.map(formatCouponDates);
